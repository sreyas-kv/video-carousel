...
├── main.tsx
└── index.css
## Getting Started### Prerequisites- Node.js 18+- npm### Installationshnpm install
Development
npm run dev
...
Outputs to dist/.
Testing
npm test          # Run tests with coveragenpm run test:watch  # Run tests in watch mode
Usage
Basic
import { VideoCarousel } from "./components/VideoCarousel/VideoCarousel";import { VIDEOS } from "./constants/videos";function App() {  return <VideoCarousel videos={VIDEOS} />;}
With Options
<VideoCarousel  videos={VIDEOS}  title="My Videos"  visibleCount={6}/>
...
Video Type
interface Video {  id: number;  title: string;  src: string;  // URL to video file (e.g. .mp4)}
Keyboard Controls
← — Previous video
→ — Next video